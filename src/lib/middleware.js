        ...
        return async function verifyJwtMiddleware(ctx, next) {
            ...
            if (token) {
                try {
                    ...
                    ctx.state.user = await authDetails(payload, token);
                } catch (err) {
                    ...
                    try {
                        ...
                        if ([ 'invalid token', 'invalid signature', 'jwt malformed' ].includes(err.message)) {
        ...
        return async function verifyJwtMiddleware(ctx, next) {
            ...
            if (token) {
                try {
                    ...
                    // valid token: accept it...
                    ctx.state.auth = await authDetails(payload);
                } catch (err) {
                    if ([ 'invalid token', 'invalid signature', 'jwt malformed' ].includes(err.message)) {
                        // delete the cookie holding the JSON Web Token
                        ctx.cookies.set('koa:jwt', null, options);
...
async function authDetails(jwtPayload, token) {
